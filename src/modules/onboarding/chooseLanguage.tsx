  ...
  return (
    <div className="w-full h-full flex justify-center items-center">
      <div className="flex flex-col gap-6 items-center">
          ...
          {languages.map((language: any) => (
            <div
              ...
              className={`w-[280px] h-10 border rounded-xl flex justify-center items-center gap-3 cursor-pointer transition-all ${
                selectedLanguage === language.value ? "border-primary" : "border-[#DADADA] hover:border-gray-400"
              }`}
        ...
        {selectedLanguage && (
          <Button onClick={handleContinue} className="w-[280px] h-10! rounded-xl! text-white! bg-primary! mt-4">Continue</Button>
        )}
      </div>
    </div>
  );
